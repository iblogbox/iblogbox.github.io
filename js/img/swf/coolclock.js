...
function fillnumber(s){
	...
	render: function(hour,min,sec,now) {
		// Get the skin
		...
		if(now && clockDigital){
			clockDigital.innerHTML=this.dateTimeFormat(now);
		}
		// Write the time
		if (this.showDigital) {
			this.drawTextAt(
	...
	refreshDisplay: function() {
		...
		if (this.gmtOffset != null) {
			...
			this.render(offsetNow.getUTCHours(),offsetNow.getUTCMinutes(),offsetNow.getUTCSeconds(),now);
		}
		else {
			// Use local time
			this.render(now.getHours(),now.getMinutes(),now.getSeconds(),now);
		}
	},
